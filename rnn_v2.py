...
def main(args):
	...
	if args.test:
		...
		basic_cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(num_units=n_neurons, activation=tf.nn.relu)
		rnn_outputs, states= tf.compat.v1.nn.dynamic_rnn(basic_cell, X, dtype=tf.float32) 
		
		stacked_rnn_ouput= tf.reshape(rnn_outputs, [-1,n_neurons])
		stacked_outputs=tf.compat.v1.layers.dense(stacked_rnn_ouput, n_outputs)
		...
		with tf.compat.v1.Session() as sess:
			...
			for i in range(ori_test_len):
				if prediction[i]:
					...
					tn+=1.0
			precs=tp/(tp+fp)*100  #precission
			recc=tp/(tp+fn)*100   #recall
			accu=(tp+tn)/(tp+tn+fp+fn)*100  # accuracy
